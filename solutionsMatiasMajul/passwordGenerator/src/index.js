...
const wordsCheck = document.querySelector("#words-check");
...
function fetchData(API) {
  axios.get(API).then((response) => {
    words = response.data.quotes.map((quote) => quote.text);
...
inputLength.addEventListener("input", (e) => {
  ...
  generatePassword(passwordLengthParagraph.innerText, checks);
});

wordsCheck.addEventListener("change", (e) => {
  if (e.target.checked) {
